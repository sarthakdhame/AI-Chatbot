...
const createMessageElement = (content, ...classes) => {
    ...
    const classList = classes.flatMap(cls => cls.split(' ').filter(Boolean));
    div.classList.add('message', ...classList);
    div.innerHTML = content;
    return div;
};

// Load and display saved chat history on page load
const loadChatHistory = () => {
    chatHistory.forEach(chat => {
        if (chat.role === 'user') {
            const messageContent = `<div class="message-text"></div>${chat.image ? `<img src="${chat.image}" class="attachment" />` : ''}`;
            const messageDiv = createMessageElement(messageContent, 'user-message');
            messageDiv.querySelector('.message-text').textContent = chat.content;
            ...
            const messageDiv = createMessageElement(messageContent, 'bot-message', 'message-bot-message');
            messageDiv.querySelector('.message-text').textContent = chat.content;
            chatBody.appendChild(messageDiv);
        }
    });
...
const generateBotResponse = async (incomingMessageDiv) => {
    ...
    const requestOptions = {
        ...
        body: JSON.stringify({
            model: userData.file.data ? "meta-llama/llama-4-scout-17b-16e-instruct" : "llama-3.3-70b-versatile",
...
const handleOutgoingMessage = (e) => {
    ...
    chatHistory.push({
        ...
        image: userData.file.data ? `data:${userData.file.mime_type};base64,${userData.file.data}` : null
    });
    saveChatHistory();
    ...
    setTimeout(() => {
        ...
        const incomingMessageDiv = createMessageElement(messageContent, "bot-message", "message-bot-message", "thinking");
...
document.querySelector('.chat-form').appendChild(pickerContainer);
